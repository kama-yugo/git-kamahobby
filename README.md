...
画面最上端はシステムのステータスバーが最前面を占有しており、アプリのオーバーレイ
はそれより下のレイヤーになるため、ごく上端のスワイプは通知シェードに奪われます。
本アプリではトリガー帯を**ステータスバーのすぐ下**に配置しているので、
**最上端ぴったりではなく、少し下から下方向にスワイプ**してください。

確実な方法として、**常駐通知をタップ（または通知の「パネルを開く」）するとパネルが
開きます**。まずはこれで表示を確認してください。なお、どのアプリの上でも完全に
「最上端スワイプ」で開きたい場合はアクセシビリティサービスによるジェスチャー検出が
必要になります（必要であれば追加可能です）。

### 「他のアプリの上に表示」のトグルが ON にできない / グレーアウト

Android 13 以降の **「制限付き設定」(Restricted settings / Enhanced Confirmation Mode)** によるものです。
ストア外から入れた（サイドロードした）アプリは、この高リスク権限のトグルが既定でロックされます。
**アプリのバグではありません。** 次の手順で解除してください：

1. **設定 → アプリ → Galaxy Quick Panel** を開く
2. 右上の **⋮（三点メニュー）→「制限付き設定を許可」(Allow restricted settings)**
3. 戻って「他のアプリの上に表示」を ON

⋮ メニューに項目が出ないときは、一度トグルを押して「セキュリティのため現在この設定は使用できません」を表示させてから、アプリ情報の ⋮ を開くと出てきます。
アプリ内で「許可する」を押すと、この手順を案内するダイアログも表示されます。
